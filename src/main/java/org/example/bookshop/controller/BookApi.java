...
public interface BookApi {
    ...
    @Operation(
        ...
        description = "Фильтры: q (поиск по названию), categoryId, minPrice, maxPrice. Все фильтры опциональны. " +
            "Конфликтные диапазоны (minPrice > maxPrice) возвращают пустой результат, а не ошибку."
    )
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Страница книг, удовлетворяющих фильтрам")
    })
    ResponseEntity<PageResponse<BookDto>> search(@ModelAttribute BookFilterRequest filter, Pageable pageable);
